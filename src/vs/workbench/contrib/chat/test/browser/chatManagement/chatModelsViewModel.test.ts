...
import { ChatModelGroup, ChatModelsViewModel, ILanguageModelEntry, ILanguageModelProviderEntry, isLanguageModelProviderEntry, isLanguageModelGroupEntry } from '../../../browser/chatManagement/chatModelsViewModel.js';
...
suite('ChatModelsViewModel', () => {
	...
	test('should fire onDidChangeGrouping when grouping changes', () => {
		...
		viewModel.groupBy = ChatModelGroup.Vendor;
		// Should not fire since it's already Vendor
		assert.strictEqual(fired, false);
	});

	test('should sort models within vendor groups', async () => {
		languageModelsService.addVendor({
			vendor: 'anthropic',
			displayName: 'Anthropic',
			managementCommand: undefined,
		...
		// Models should be sorted within their vendor groups
		assert.ok(actuals.length > 0);
	});
